...
<div id={collection.name} class="bg-white dark:bg-black-700 rounded-xl mb-4 shadow-sm border border-black-200 dark:border-black-500">
	<header class="flex gap-4 items-center justify-between mb-4 p-4 pb-0">
		<div class="flex gap-2 items-center">
			...
			<div class="">
				<span class="p-2 text-sm rounded bg-purple-100 text-purple-950">{collection.files.length} File</span>
			</div>
		</div>
		<button
			on:click={() => deleteCollection(collection.name)}
			class=" w-8 h-8 flex justify-center items-center rounded bg-black-100 dark:bg-black-600 hover:bg-black-200 transition-all"
		>
			<Trash class="w-4" />
		</button>
	</header>
	<div class="px-4 pb-4">
		<div class="ml-2 pl-2 border-l border-black-200 dark:border-black-500">
			{#each collection.files as file}
				<div
					class="group flex justify-between gap-4 items-center p-2 hover:bg-black-100 dark:hover:bg-black-800 transition-colors border-b border-black-200 dark:border-black-500 hover:rounded-lg hover:border-transparent text-sm  last:border-transparent"
				>
					<div class="flex items-center gap-2">
						<div class="">
							<h3 class="font-semibold bg-white dark:bg-black-800 shadow rounded border border-black-200 dark:border-black-700 p-1 px-2">
								{file.name}
							</h3>
						</div>
						<div class="">
							<span class="p-2 text-sm rounded bg-black-100 dark:bg-black-800">{file.chunks} Chunks</span>
						</div>
					</div>
					<button
	...
	<footer class="p-4 border-t border-black-200 dark:border-black-500">
		<p class="text-sm mb-2">Context for</p>
		<div class="flex gap-2 items-center">
			{#each usedBy as id}
				<a
					href={`/chat/${id}`}
					class="text-sm flex gap-1 items-center p-2 shadow rounded bg-white dark:bg-black-700 border border-black-200 dark:border-black-500"
					>{id} <Url class="w-4" />
				</a>
			{/each}
			<div class="relative">
