...
<div
	class="
  {$page.url.pathname.includes('chat') || isHome
		? 'bg-white dark:bg-black-800 border-black-200 dark:border-black-700 shadow-lg'
		: 'hover:bg-black-100 border-transparent dark:hover:bg-black-800'}
   border rounded-2xl
...
>
	<a
		href="/"
		class="
    {isHome ? 'bg-black-100 dark:bg-black-700 ' : 'hover:bg-black-100 dark:hover:bg-black-700'}
	...
	>
		...
		<span
			class="
    {isHome ? 'bg-black-200 dark:bg-black-700 ' : 'dark:bg-black-700 bg-black-100'}
     p-1 rounded text-xs">⌘+o</span
		>
	</a>
	{#if $history.length > 0}
		<div class="border rounded-lg border-black-200 dark:border-black-600 p-1 flex gap-2">
			<Search />
